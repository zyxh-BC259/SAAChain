...
def sample_ensemble(config):
	...
	offset_xy = tf.random.uniform([N, 2], minval=0, maxval=config.offset_max + 1, dtype=tf.int32)
			
	# Sample scale level.
	cumulative_sum = np.cumsum(config.scale_probabilities)
	u = cumulative_sum[-1] * tf.random.uniform([])
	...
	# Scale randomization.
	scale_offset_xy = tf.random.uniform([2], minval=0, maxval=scale_level, dtype=tf.int32)
